...
def get_table_size(table_name):
    ...
    with conn:
        return c.execute("SELECT COUNT(*) from {0}".format(table_name)).fetchone()[0]
...
def update_player(player_id, ranking):
    """Update player ranking"""
    with conn:
        new_ranking = (ranking, player_id)
        c.execute("UPDATE player SET ranking = ? WHERE player_id = ?", new_ranking)
...
QUERY_PLAYERS = """SELECT player_id, last_name, first_name
                   FROM player
                   ORDER BY last_name, first_name"""

"""query to get the player list sorted by ranking"""
QUERY_RANKED_PLAYERS = """SELECT player_id, last_name, first_name, ranking
                          FROM player
...
def display_ranked_player(query=QUERY_RANKED_PLAYERS):
    ...
    with conn:
        ...
        for last_first_rank in (name.fetchall()):
            print('{0:<5d}{1:<20s}{2:<4d}'.format(last_first_rank[0], ' '.join(last_first_rank[1:3]), last_first_rank[3]))
        print(40 * '-', '\n')


"""Query to get all the recorded tournaments"""
QUERY_TOURS = """SELECT rowid, name, venue, date
                   FROM tournaments
                   ORDER BY date"""
...
def display_tour(query=QUERY_TOURS):
    ...
    with conn:
        name = c.execute(query)
        print(37 * '-')
        print('{0:<4s}{1:<16s}{2:<16s}{3:<16s}'.format('Tid', 'Name', 'Venue', 'Date'))
        print(37 * '-')
        for tour in (name.fetchall()):
            print('{0:<4d}{1:<16s} {2:<16s}{3:<16s}'.format(tour[0], tour[1], tour[2], tour[3]))
        print(37 * '-', '\n')
    ...
    with conn:
        ...
        for match in (result.fetchall()):
            print('{0:20s}{1:<7d}{2:<10s}{3:<20s}{4:<20s}'.
                  format(match[6], match[0], match[1], ' '.join(match[2:4]), ' '.join(match[4:6])))
    print("\n")


def exist_player(last_name):
    with conn:
        name = c.execute(f"SELECT first_name, last_name FROM player\
